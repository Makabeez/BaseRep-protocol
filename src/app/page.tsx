...
export default function LeaderboardPage() {
  ...
  const handleAttest = async () => {
    ...
    try {
      ...
      alert("DNA Certifié on-chain ! UID: " + uid);
    } catch (err) {
      console.error(err);
      alert("Erreur lors de l'attestation.");
    } finally {
          ...
          {LEADERBOARD_DATA.map((item) => (
            <div key={item.id} className="flex justify-between items-center p-7 border-b border-zinc-900/50 last:border-0 hover:bg-zinc-800/10 transition-all">
              ...
              <div className="flex flex-col items-end">
                ...
                <span className="text-[9px] font-black text-zinc-700 tracking-widest uppercase font-bold">Points</span>
              </div>
            </div>
          ))}
        </div>
