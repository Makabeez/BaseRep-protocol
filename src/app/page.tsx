...
const ABI = [{ name: 'mintReputation', type: 'function', stateMutability: 'nonpayable', inputs: [{ name: 'score', type: 'uint256' }], outputs: [] }] as const;
...
export default function Home() {
  ...
  const handleMint = () => {
    ...
    writeContract({
      abi: ABI,
      address: '0x2f225F8A538e7fD613e8ba79DCDdC7D1422AEd1C', // Contrat temporaire
  ...
  return (
    <main className="min-h-screen bg-black text-white flex flex-col items-center justify-center p-6">
      <div className="absolute top-6 right-6"><ConnectButton /></div>
      ...
      <div className="mt-12 w-full max-w-md flex flex-col gap-4">
        <div className="relative">
          <input type="text" value={address} onChange={(e) => setAddress(e.target.value)} placeholder="0x..." className="w-full bg-zinc-900 border border-zinc-800 p-5 rounded-2xl font-bold outline-none" />
          <button onClick={handleCheck} className="absolute right-2 top-2 bottom-2 bg-white text-black px-6 rounded-xl font-black uppercase text-xs">{loading ? '...' : 'Check'}</button>
        </div>
        {data.ethosScore > 0 && (
          <button onClick={handleMint} className="w-full bg-blue-600 hover:bg-blue-700 text-white py-5 rounded-3xl font-black uppercase tracking-widest transition-all">
